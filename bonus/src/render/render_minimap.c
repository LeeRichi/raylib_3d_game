...
/*   Updated: 2025/07/09 16:42:06 by wweerasi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_sq_block(t_game *game, int x, int y, uint32_t clr)
{
	...
	blk_size = 16;
	blk_y = 1;
	blk_stpos = ((game->img->height - y * blk_size)
			* (int)game->img->width) + (x * blk_size);
...
static uint32_t	set_block_color(t_game *game, int i, int j)
{
	char		**map;
	t_player	*p;
	...
	if (i < 0 || j < 0 || i >= game->map->coloum_count
		...
		return (0xFF0000FF);
	else if (map[j][i] == '0' || map[j][i] == 'N' || map[j][i] == 'S'
		|| map[j][i] == 'E' || map[j][i] == 'W')
		return (0xFFFFFFFF);
	else
		return (0xFFB5BEB2);
...
void	render_minimap(t_game *game, t_player	*p, int x, int y)
{
	...
	while (j < (int) floor(p->y) + rad + 1)
	{
		...
		{
			color = set_block_color(game, i, j);
